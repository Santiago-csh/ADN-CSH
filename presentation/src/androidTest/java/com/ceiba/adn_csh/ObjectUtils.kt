...
class ObjectUtils {
    ...
    companion object{
        ...
        fun fillIdEditText(id:Int, text:String){
            onView(ViewMatchers.withId(id)).perform(ViewActions.replaceText(text))
        }
        ...
        fun verifyDisplayToastCondition(textSuccess: String, textError: String, decorView: View){
            if(!onView(ViewMatchers.withText(textSuccess)).inRoot(RootMatchers.withDecorView(Matchers.not(decorView))).isDisplayed()){
